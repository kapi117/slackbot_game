...
def send_message(
    ...
    channels: List[str],
    client: WebClient,
    thread_ts: Optional[List[str]] = None,
...
def send_message_to_everyone_in_channel(
    message: str, channel: str, client: WebClient, metadata: object = None
):
    """
...
def schedule_message_to_everyone_in_channel(
    message: str,
    channel: str,
    time: datetime,
    client: WebClient,
    ...
    for user in users:
        messages.append(
            send_scheduled_message(message, user, time, client, metadata=metadata)
        )
    return messages
